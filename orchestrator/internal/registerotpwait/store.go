...
const defaultKeyPrefix = "byte-v-forge:gpt:register-protocol-otp-wait"
...
func NewStore(client redis.Cmdable, keyPrefix string) (*Store, error) {
	if client == nil {
		return nil, errors.New("register protocol otp wait redis client is required")
	}
...
func (s *Store) Register(ctx context.Context, entry Entry, ttl time.Duration) error {
	if s == nil {
		return errors.New("register protocol otp wait store is required")
	}
...
func (s *Store) PendingForEmails(ctx context.Context, emails []string, receivedAtUnix int64) ([]Entry, error) {
	if s == nil {
		return nil, errors.New("register protocol otp wait store is required")
	}
...
func (s *Store) Get(ctx context.Context, jobID string) (Entry, bool, error) {
	if s == nil {
		return Entry{}, false, errors.New("register protocol otp wait store is required")
	}
...
func (s *Store) Claim(ctx context.Context, jobID string, ttl time.Duration) (bool, error) {
	if s == nil {
		return false, errors.New("register protocol otp wait store is required")
	}
	jobID = strings.TrimSpace(jobID)
	if jobID == "" {
		return false, nil
...
func (e Entry) validate() error {
	...
	if len(missing) > 0 {
		return fmt.Errorf("register protocol otp wait missing %s", strings.Join(missing, ", "))
	}
	return nil
}
