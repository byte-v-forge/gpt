...
export function GptAccountsPage() {
  ...
  const selectedIndex = data.selected ? data.accounts.findIndex((account) => account.account_id === data.selected?.account_id) : -1;
  const selectedPhoneState = data.selected ? accountCodexPhoneState(data.selected, data.jobs, selectedIndex >= 3) : null;
