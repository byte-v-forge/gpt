...
export function GptAccountsPage() {
  ...
  return (
    <>
      ...
      <GptAccountsView accounts={data.accounts} jobs={data.jobs} mailboxes={data.mailboxes} allocations={data.allocations} mailboxDomains={data.domains} selectedAccountId={selectedAccountID} showSecrets={showSecrets} busy={busy} mailboxSyncing={actions.syncingMailboxes} cleaningInvalidAccounts={actions.cleaningInvalidAccounts} runningAccountIds={data.runningIds} runningWorkflowByAccountID={data.runningByAccount} onCreateDone={async (message) => { actions.toast.showOK(message); await data.invalidate(); }} onError={actions.toast.showError} onToggleSecrets={() => setShowSecrets((value) => !value)} onSyncMailboxes={actions.syncMailboxes} onCleanInvalidAccounts={actions.cleanInvalidAccounts} onSelectAccount={(account) => setSelectedAccountID(account.account_id)} onOpenWorkflow={() => actions.toast.showOK('请在工作流页查看任务详情')} onCancelWorkflow={cancelWorkflow} onRegisterProtocol={(account) => actions.runWorkflow('协议注册', '/api/workflows/register-protocol', account)} onLoginProtocol={(account) => actions.runWorkflow('协议登录', '/api/workflows/login-protocol', account)} onCodexOAuthProtocol={(account) => actions.runWorkflow('协议 auth.json', '/api/workflows/codex-oauth-protocol', account)} onCodexOAuthBatchAddPhone={actions.runCodexOAuthBatchAddPhone} onGoPayPayment={(account, channel) => void actions.runGoPayPayment(account, channel)} onSubmitOTP={actions.submitJobOTP} onResendOTP={actions.resendJobOTP} onConfirmManualPayment={confirmManualPayment} onSelectAddBalance={selectAddBalance} onConfirmAddBalance={confirmAddBalance} />
      <DetailDrawer open={!!data.selected} title="GPT账号详情" onClose={() => setSelectedAccountID('')}>
        {data.selected && <AccountDetails account={data.selected} showSecrets={showSecrets} busy={busy} inboxLoading={actions.inboxLoading} mailboxContext={mailboxContextForEmail(data.mailboxes, data.allocations, data.selected)} latestOtp={latestOtpForEmail(actions.inbox, data.mailboxes, data.selected.email) || latestOtpFromAccount(data.selected)} activationChannel={accountActivationChannel(data.selected, data.jobs)} codexPhoneState={selectedPhoneState!} refreshingAccessToken={actions.refreshing.has(data.selected.account_id)} onCopy={actions.toast.copyValue} onFetchInbox={actions.fetchInbox} onSessionSave={(account, sessionToken) => actions.updateAccount(account, { session_token: sessionToken }, '认证信息已更新')} onAccessSave={(account, accessToken) => actions.updateAccount(account, { access_token: accessToken }, '认证信息已更新')} onProbeAccount={(account) => actions.runWorkflow('探测账号', '/api/workflows/probe', account)} onRegister={(account) => actions.runWorkflow('浏览器注册', '/api/workflows/register', account)} onRegisterProtocol={(account) => actions.runWorkflow('协议注册', '/api/workflows/register-protocol', account)} onLogin={(account) => actions.runWorkflow(loginActionLabel(account), '/api/workflows/login', account)} onLoginProtocol={(account) => actions.runWorkflow('协议' + loginActionLabel(account), '/api/workflows/login-protocol', account)} onCodexOAuthAddPhone={(account) => actions.runWorkflow('生成 auth.json', '/api/workflows/codex-oauth', account)} onCodexOAuthProtocol={(account) => actions.runWorkflow('协议 auth.json', '/api/workflows/codex-oauth-protocol', account)} onGoPayPayment={(account, channel) => void actions.runGoPayPayment(account, channel)} onRefreshAccessToken={actions.refreshAccessToken} onDelete={actions.deleteAccount} />}
      </DetailDrawer>
    </>
  );
}
